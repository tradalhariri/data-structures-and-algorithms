...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let targcetObj = arr.find(obj => obj.name === character)
  let values = Object.values(targcetObj); 
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  let targcetObj = arr.find(obj => obj.name === character)
  let keys = targcetObj? Object.keys(targcetObj):false;

  return  keys!==false? (keys.includes('children')?true:false):false;
...
const totalCharacters = (arr) => {
  ...
  let total = arr.reduce((acc,obj) =>{
      let keys = Object.keys(obj);
      let charactersObj = keys.reduce((acc2,key)=>{
               if(key !== 'house'){
                 if(key === 'children'){
                  //  console.log( acc2+(obj[key].length));
                   acc2 = acc2+(obj[key].length);
                 }
...
const houseSize = (arr) => {
  ...
  // Solution code here...
    ...
    return {house:obj.house,members:charactersObj};
})
  return sizes;
};
  ...
  let survivors = arr.map(obj => {
    ...
    let charactersObj = keys.reduce((acc2,key)=>{
             if(key !== 'house'){
               ...
               else if(obj[key] !== null && !deceasedSpouses.includes(obj[key])){
                 acc2=acc2+1;
               }
             }
             return acc2;
