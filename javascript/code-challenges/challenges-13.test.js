...
const findEvery = (arr, target) => {
  // Solution code here...
  return arr.find(str => !str.includes(target))=== undefined?true:false;
};
...
const unenrollBrook = (arr) => {
  ...
  return arr.map(row => {
       for (let i = 0; i < row.length; i++){
         let j = i;
         while(j === i && j<row.length){
          if(row[i].includes('Brook')){
            row.splice(i, 1);
          }
          else{
           j=j+1;
...
const sortByDay = (arr) => {
  ...
  for(let i=0; i<7; i++) {
    let day = [];
    let arr1 = arr[i]?arr[i].split(' '):[];
    for(let j=0; j<7; j++) {
       for(let k=0; k<arr1.length; k++) {
         if(daysOfWeek[j]=== arr1[k]){
           day.push(arr[i])
           break;
         }
       }
