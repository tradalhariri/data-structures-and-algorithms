...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((acc,character,index) => {
   acc[index]=character.name ;
   return acc;
  },[]);
};
...
const calculateAverage = (arr) => {
  // Solution code here...
  let accObj = arr.reduce((acc,number) => {
   acc.count+=1;
   acc.sum+=number;
   return acc;
  },{count:0,sum:0});
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  return arr.reduce((acc,number)=> isPrime(number)?(acc+1):acc,0);
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce((acc,obj)=>  (obj.stat.name === statName?obj:acc),{})
};
...
const extractChildren = (arr) => {
  // Solution code here...
  let filtered = arr.filter((obj)=> [...obj.name].includes('a'));
  return filtered.reduce((acc,obj)=>{
   acc= obj.children ? acc.concat(obj.children) : acc ;
    return  acc ;
  } ,[])
};
