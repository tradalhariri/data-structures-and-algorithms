...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  // return filterArray.map(obj => obj.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let filteredOnlyNumeric = arr.filter(element => (typeof element === 'number'));
  return filteredOnlyNumeric.map(number => (number % 2 === 0?'even':'odd'));
};
