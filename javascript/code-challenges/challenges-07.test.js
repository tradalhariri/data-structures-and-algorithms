...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
 return starWarsArr.sort((person1,person2)=> Number(person2.height)-Number(person1.height))
}
...
const howMuchPencil = (str) => {
  ...
  for (let i in str) {
    ...
    let sliced = splittedArr.slice(i);
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  result = recipe.ingredients.map(recipeAmount =>{
   let firstSpace= recipeAmount.indexOf(' ')
   let recipeAmountWithoutFirstSpace = recipeAmount.slice(firstSpace+1);
   let secondSpace = recipeAmountWithoutFirstSpace.indexOf(' ');
   let recipeAmountWithoutSecondSpaceb= recipeAmountWithoutFirstSpace.slice(secondSpace+1);
   return recipeAmountWithoutSecondSpaceb; 
  })
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  result =recipe.ingredients.map(recipeAmount => {
    let splitedRecipe = recipeAmount.split(' ');
    let food = splitedRecipe.slice(2).join(' ');
    return food;
  })
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  let afterRemoveCharacters ;
  afterRemoveCharacters = numberOfCharacters > str.length?'':(numberOfCharacters < 0 ?str:true);
  afterRemoveCharacters = afterRemoveCharacters === true?str.slice(0,str.length-numberOfCharacters) :afterRemoveCharacters;

  return afterRemoveCharacters;
};
...
const totalSumCSV = (str) => {
  let total = 0;
  // Solution code here...
  total = str.split(',').reduce((acc,number)=> Number(number)+acc,0);
  return total;
};
...
const removeVowels = (str) => {
  ...
  let strWithoutVowels =  strToArray.filter(char => !vowels.includes(char) ).join('');
...
const extractVowels = (str) => {
  ...
  let vowels = ['a','e','o','u','i'];
  let strWithoutVowels =  strToArray.filter(char => !vowels.includes(char) ).join('');
  let removedVowels = strToArray.filter(char => vowels.includes(char) ).sort().join('');

  result.push(strWithoutVowels);
  result.push(removedVowels);
  return result;
